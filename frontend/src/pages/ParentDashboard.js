...
import { Calendar, BookOpen, Award, Bell, AlertCircle, RefreshCw, TrendingUp } from 'lucide-react';
    ...
    <Layout>
      <div className="space-y-6">
        ...
        <div className="bg-gradient-to-r from-purple-600 to-pink-600 rounded-xl p-6 text-white">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-purple-200 text-sm mt-1">Monitor your child's academic progress</p>
            </div>
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
          <StatCard icon={Calendar} label="Attendance"    value={`${attRate}%`}  sub={`${present} / ${attendance.length} days`} color="blue" />
          <StatCard icon={BookOpen} label="Pending HW"    value={pending.length} sub={`${homework.length} total assigned`}       color="green" />
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-5">
            <h3 className="font-semibold text-gray-900 mb-4 flex items-center gap-2">
              <TrendingUp className="h-4 w-4 text-purple-600" /> Performance Summary
            </h3>
            {marks.length === 0 ? (
                  ...
                  return (
                    <div key={m.id}>
                      <div className="flex justify-between text-sm mb-1">
                        <span className="text-gray-700">Subject #{m.subject_id}</span>
          ...
          <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-5">
            <h3 className="font-semibold text-gray-900 mb-4 flex items-center gap-2">
              <Bell className="h-4 w-4 text-orange-600" /> School Notifications
            </h3>
            {notifications.length === 0 ? (
              ...
              <div className="space-y-2">
                {notifications.slice(0, 5).map(n => (
                  <div key={n.id} className={`p-3 rounded-lg ${n.is_read ? 'bg-gray-50' : 'bg-purple-50 border border-purple-100'}`}>
        ...
        {pending.length > 0 && (
          <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-5">
            <h3 className="font-semibold text-gray-900 mb-4 flex items-center gap-2">
              <BookOpen className="h-4 w-4 text-green-600" /> Child's Pending Homework
            </h3>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
              {pending.slice(0, 4).map(hw => (
                <div key={hw.id} className="flex items-center justify-between p-3 bg-yellow-50 border border-yellow-100 rounded-lg">
                  <div>
        ...
        {/* AI CTA */}
        <div className="bg-gradient-to-r from-purple-600 to-pink-600 rounded-xl p-5 text-white flex items-center justify-between">
          <div>
            <h3 className="font-semibold">AI Parent Assistant</h3>
            <p className="text-purple-200 text-sm mt-0.5">Get insights on your child's performance and recommendations</p>
          </div>
          <a href="/ai-chat" className="px-4 py-2 bg-white text-purple-600 rounded-lg font-medium text-sm hover:bg-gray-100 transition-colors">
            Chat Now
          </a>
        </div>
      </div>
